...
script.on_event(defines.events.on_gui_click, function(event)
	...
	elseif element.name == "CCEditCombinator" then
		...
		if combinator.output then
			...
			CCContainer.editCombinatorContainer.durationRow.style.visible = (combinator.output.type == "duration")
		else
			combinator.output = {
				signalNum = 1,
				amount = 1,
...
function createGUI(player)
	...
	local durationRow = editCombinatorComtainer.add{type="flow", direction="vertical", name="durationRow"}
	setStyles(durationRow, {
		visible = false
	})

	setStyles(durationRow.add{type="label", caption="Signal Duration (Ticks)"}, {
		top_padding = 30,
		font = "default-large-bold"
	})
	durationRow.add{type="textfield", name="editCombinatorDuration"}
